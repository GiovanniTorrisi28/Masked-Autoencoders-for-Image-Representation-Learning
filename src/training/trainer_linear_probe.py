...
class LinearProbeTrainer:
    """
    Training loop for linear probing on top of a frozen MAE encoder.

    The encoder must already be frozen (freeze_encoder() called before passing
    the model here). Only the linear classification head parameters are updated.

    Args:
        model: ViTClassifier with frozen encoder
        optimizer: optimizer (operates only on head parameters)
    ...
    def train_one_epoch(self, epoch: int) -> dict[str, float]:
        self.model.train()
        # Keep encoder in eval mode to disable dropout in frozen layers
    ...
    def train(self, start_epoch: int = 0) -> None:
        print(f"\n{'='*60}")
        print(f"  Linear Probe Training: {self.epochs} epochs")
        print(f"{'='*60}\n")
        ...
        print(f"\nLinear probe complete. Best Val Top-1: {best_top1:.2f}%")
